...
function buildHtml({ name, business, email, phone, planLabel }) {
...
module.exports = async function handler(req, res) {
  ...
  const { name, business, email, phone, plan } = body || {};
  ...
  if (!process.env.SMTP_USER || !process.env.SMTP_PASS) {
    res
      .status(500)
      .json({ error: "Servidor de email não configurado. Contacte o suporte." });
    return;
  }
  ...
  try {
    await transporter.sendMail({
      ...
      html: buildHtml({ name, business, email, phone, planLabel }),
    });
    ...
    res
      .status(500)
      .json({ error: "Erro ao enviar pedido. Por favor tente novamente." });
  }
};
